function [ projected_pg ] = compute_pg_projection_SE3(uh, projection_time, varargin)
...
if strcmp(intent_type, 'conf')
    pg = compute_conf_SE3(uh, xr_T); %confidence function does not care of what the current pg is and therefore ignores pg0
...
%     projection_time = 1; %let the DS evolve for projection_time seconds
    ...
    for i=1:length(t_stamps)-1
        pgs_project(:, i+1) = compute_p_of_g_dft_SE3(uh, xr_T, pgs_project(:, i));
        xr_T = sim_kinematics_SE3(xr_T, uh);
    end
...
%     projection_time = 1;
    ...
    for i=1:length(t_stamps)-1
        pgs_project(:, i+1) = compute_bayes_SE3(uh, xr_T, pgs_project(:, i));
        xr_T = sim_kinematics_SE3(xr_T, uh);
    end
    pg = pgs_project(:, end);
end
