function [ projected_pg ] = compute_pg_projection_SE2(uh, projection_time, varargin)
...
if strcmp(intent_type, 'conf')
    pg = compute_conf_SE2(uh, curr_x); %confidence function does not care of what the current pg is and therefore ignores pg0
...
%     projection_time = 1; %let the DS evolve for projection_time seconds
    ...
    for i=1:length(t_stamps)-1
        ...
        curr_x = sim_kinematics_SE2(curr_x, uh);
    end
