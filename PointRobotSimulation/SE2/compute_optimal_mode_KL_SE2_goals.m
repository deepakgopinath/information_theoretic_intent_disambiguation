function [ best_mode ] = compute_optimal_mode_KL_SE2_goals( intent_type, xr_T, pg )
...
for i=1:length(cm)
    ...
    if length(curr_mode) == 3
        for j=1:size(candidate_uhs, 2)
            ...
            EID_AR(i) = EID_AR(i) + (1/size(candidate_uhs, 2))*compute_projected_kl_SE2(uh_t, intent_type, xr_T, pg);
        end
...
function uh_r =  generate_rotation(xg, xr)
    uh_r = 0; %if angles are same, don't turn. 
    if xr < xg
        if abs(xg-xr) <= pi
