function [ best_mode ] = compute_optimal_mode_KL_R2(intent_type, xr_T, pg)
...
for i=1:length(cm)
    ...
    if length(curr_mode) == 3 %mode with 3 dimensions per mode here. 
        ...
        for j=1:size(combinations, 1) % 8 combinations
            ...
            EID_AR(i) = EID_AR(i) + (1/size(combinations, 1))*compute_projected_kl_R2(uh_t, intent_type, xr_T, pg);
        end
    ...
    elseif length(curr_mode) == 1
        ...
        for j=1:length(possible_vel)
            ...
            EID_AR(i) = EID_AR(i) + (1/length(possible_vel))*compute_projected_kl_R2(uh_t, intent_type, xr_T, pg);
        end
    end
end
