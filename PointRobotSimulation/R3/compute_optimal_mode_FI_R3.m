function [ best_mode ] = compute_optimal_mode_FI_R3(intent_type, xr_T, pg)
...
for i=1:length(cm)
    ...
    if length(curr_mode) == 3 %mode with more than one dimension, at max 3 dimensions per mode here. 
        ...
        for j=1:size(combinations, 1) % 8 combinations
            ...
            EID_AR(i) = EID_AR(i) + (1/size(combinations, 1))*det((1/sig^2)*(NumHessian(@compute_projected_entropy_R3, uh_t, intent_type, xr_T, pg)));
        end
    ...
    elseif length(curr_mode) == 1
        ...
        for j=1:length(possible_vel)
            ...
            EID_AR(i) = EID_AR(i) + (1/length(possible_vel))*det((1/sig^2)*(NumHessian(@compute_projected_entropy_R3, uh_t, intent_type, xr_T, pg)));
        end
    end
end
